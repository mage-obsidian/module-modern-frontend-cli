...
`mage-obsidian:frontend:doctor`'s Adobe Commerce section needs
`MageObsidian\ModernFrontend\Service\Dev\AdobeCommerceInventory`, shipped in
`mage-obsidian/module-modern-frontend` 2.20.0 — the floor `composer.json`
already requires. The command resolves that class through DI, so an older
framework makes it fail on construction rather than degrade.
